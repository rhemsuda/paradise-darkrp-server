...
function ENT:Initialize()
    ...
    self.sparking = false
    self.health = 150 -- Increased health for donator printer
    ...
    -- Start money generation timer (every 45 seconds for donator printer)
...
function ENT:CreateMoneybag()
    ...
    if amount == 0 then
        amount = 500 -- Increased amount for donator printer
    end

    self:SetStoredMoney(self:GetStoredMoney() + amount)
    DarkRP.notify(self:Getowning_ent(), 0, 4, "Donator Printer has generated $" .. amount .. ". Total stored: $" .. self:GetStoredMoney())
...
function ENT:BurstIntoFlames()
    ...
    self:Ignite(10, 0)
    self.sparking = true
    timer.Create("FireDamage_" .. self:EntIndex(), 1, 15, function() -- Adjusted for higher health
        if not IsValid(self) then return end
...
function ENT:OnRemove()
    -- Clean up timers when the entity is removed
