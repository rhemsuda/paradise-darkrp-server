...
net.Receive("UpdateModuleAttachment", function()
    local ent = net.ReadEntity()
    if IsValid(ent) then
        ent.ModuleAttached = net.ReadBool()
        ent.ModuleSaveCounter = net.ReadUInt(8)
        ent.ModuleMaxSaves = net.ReadUInt(8)
        ent.ModuleTopUpPending = net.ReadBool()
    end
end)
...
function ENT:Initialize()
    ...
    self.ModuleMaxSaves = self.ModuleMaxSaves or 3
...
function ENT:Draw()
    ...
    local cooledText = self.ModuleAttached and ("Cooled " .. (self.ModuleSaveCounter or 0) .. "/" .. (self.ModuleMaxSaves or 3)) or ""
    ...
    cam.Start3D2D(Pos + Ang:Up() * 11.5, Ang, 0.11)
        draw.SimpleText(text, "HUDNumber5", 0, baseY + 30, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText(owner, "HUDNumber5", 0, baseY + 60, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        if cooledText ~= "" then
            draw.SimpleText(cooledText, "HUDNumber5", 0, baseY + 90, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        draw.SimpleText(moneyText, "HUDNumber5", 0, baseY + 120, Color(0, 255, 0, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
