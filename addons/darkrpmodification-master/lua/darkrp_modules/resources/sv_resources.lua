...
net.Receive("DropResource", function(len, ply)
    ...
    -- Map resourceID to the corresponding entity
    local entityName
    if resourceID == "rock" then
        entityName = "resource_item"
    else
        entityName = "resource_" .. resourceID
    end

    -- Spawn the corresponding entity for each unit of the resource
    for i = 1, amount do
        local ent = ents.Create(entityName)
        if IsValid(ent) then
            -- Set the position with a slight random offset to avoid stacking
            ent:SetPos(ply:GetEyeTrace().HitPos + Vector(math.random(-10, 10), math.random(-10, 10), 10))
            
            -- Set resource type and amount
            ent:SetResourceType(resourceID)
            ent:SetAmount(1)

            -- Apply material and color from ResourceAppearances
            local appearance = ResourceAppearances[resourceID] or { material = "", color = nil }
            if appearance.material != "" then
                ent:SetMaterial(appearance.material)
            end
            if appearance.color then
                ent:SetColor(appearance.color)
            end
            ...
            if IsValid(phys) then
                phys:SetVelocity(ply:GetAimVector() * 100 + Vector(math.random(-50, 50), math.random(-50, 50), 0))
            end
        else
            DebugPrint("[Resources Module] Failed to spawn entity '" .. entityName .. "' for resource: " .. resourceID)
        end
    end
end)
