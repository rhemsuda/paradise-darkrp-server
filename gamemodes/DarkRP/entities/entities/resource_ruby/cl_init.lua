...
function ENT:Draw()
    ...
    if not self.debugPrinted then
        print("[Debug] resource_ruby: resourceID = " .. tostring(resourceID) .. ", amount = " .. tostring(amount))
        if not resourceID or resourceID == "" then
            print("[Debug] resource_ruby: ResourceType is nil or empty!")
        end
        if not ResourceItems then
            print("[Debug] resource_ruby: ResourceItems is nil!")
        elseif not ResourceItems[resourceID] then
            print("[Debug] resource_ruby: ResourceItems[" .. tostring(resourceID) .. "] is nil!")
        end
