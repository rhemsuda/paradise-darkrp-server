...
function ENT:Initialize()
    ...
    self:SetNWString("ResourceType", "ruby") -- Set specific resource ID
    self:SetNWInt("Amount", 1)

    self:SetMaterial("models/shiny")
    self:SetColor(Color(255, 36, 0, 220))
...
function ENT:Use(activator, caller)
    ...
    print("[Debug] resource_obsidian: Use called with resourceID = " .. tostring(resourceID) .. ", amount = " .. tostring(amount))
    if resourceID and resourceID != "" and amount > 0 then
        if SERVER then
            if AddResourceToInventory then
                print("[Debug] resource_obsidian: Calling AddResourceToInventory for " .. activator:Nick() .. " with " .. resourceID .. " x" .. amount)
        ...
        print("[Debug] resource_obsidian: Invalid resourceID or amount!")
    end
end