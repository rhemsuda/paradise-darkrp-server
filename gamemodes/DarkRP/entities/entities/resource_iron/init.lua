...
function ENT:Use(activator, caller)
    ...
    print("[Debug] resource_iron: Use called with resourceID = " .. tostring(resourceID) .. ", amount = " .. tostring(amount))
    if resourceID and resourceID != "" and amount > 0 then
        if SERVER then
            if AddResourceToInventory then
                print("[Debug] resource_iron: Calling AddResourceToInventory for " .. activator:Nick() .. " with " .. resourceID .. " x" .. amount)
        ...
        print("[Debug] resource_iron: Invalid resourceID or amount!")
    end
end